  ...
  <div class="userList-wrap clearfix">
    ...
    <el-table
      ...
      <el-table-column
        ...
        prop="">
        <template slot-scope="scope">
          <span class="btn btn-danger btn-xs">{{scope.row.attackCharacteristics}}</span>
        </template>
      </el-table-column>
      <el-table-column
        label="攻击时间"
      ...
      <el-table-column
        label="攻击状态">
        <template slot-scope="scope">
          <span class="btn btn-danger btn-xs">{{scope.row.attackStatus}}</span>
        </template>
      </el-table-column>
    </el-table>
    <div class="page-wrap">
...
<script>
  export default {
    ...
    methods: {
      /** 用户列表初始化数据 **/
      InitUserList() {
        this.$http.get('../../../../../../static/json/current-report-table.json').then(res => {
