    ...
    <el-table
      ...
      <el-table-column
        ...
        <template slot-scope="scope">
          <div style="text-align: center;">{{scope.row.type.first}}</div>
          <div style="text-align: center;">{{scope.row.type.second}}</div>
        </template>
      </el-table-column>
      <el-table-column
          label="规则"
      ...
      <el-table-column
          ...
          prop="type">
        <template slot-scope="scope">
          <div class="btn btn-info btn-xs">{{scope.row.level}}</div>
        </template>
      </el-table-column>
      <el-table-column
          label="攻击是否成功"
          ...
          prop="assetsIP">
        <template slot-scope="scope">
          <div class="btn btn-danger btn-xs">{{scope.row.attached}}</div>
        </template>
      </el-table-column>
      <el-table-column
          label="是否存在后门"
      ...
      <el-table-column
        ...
        <template slot-scope="scope">
          <div>
            <span style="color: red;font-weight: bold;font-size: 14px">超级管理员</span>
          </div>
          <div>
            <span style="color:#5BC0DE;;font-weight: bold;font-size: 14px">系统管理员</span>
          </div>
        </template>
      </el-table-column>
      <el-table-column
          ...
          prop="">
        <template slot-scope="scope">
          <div class="btn btn-primary btn-xs">数据管理</div>
        </template>
      </el-table-column>
    </el-table>
    <div class="page-wrap">
...
<script>
  export default {
    ...
    methods: {
      /** 用户列表初始化数据 **/
      InitUserList() {
        this.$http.get('../../../../../../static/json/rule-manage-table.json').then(res => {
