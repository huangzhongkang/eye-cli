    ...
    <el-table
      ...
      <el-table-column
        ...
        <template slot-scope="scope">
          <div class="btn btn-info btn-xs">{{scope.row.agreement}}</div>
        </template>
      </el-table-column>
      <el-table-column
          width="130"
          ...
          prop="type">
        <template slot-scope="scope">
          <div class="btn btn-danger btn-xs">{{scope.row.type}}</div>
        </template>
      </el-table-column>
      <el-table-column
          label="资产IP"
      ...
      <el-table-column
          ...
          prop="userOperation">
        <template slot-scope="scope">
          <div @click="dealWidth" v-if="deal_width === false" class="btn btn-danger btn-sm">处理</div>
          <div @click="dealWidth" v-if="deal_width === true" class="btn btn-danger btn-sm disabled">已处理</div>
        </template>
      </el-table-column>
    </el-table>
    <div class="page-wrap">
...
<script>
  export default {
    ...
    methods: {
      /** 用户列表初始化数据 **/
      InitUserList() {
        this.$http.get('../../../../../../static/json/statis-search-port.json').then(res => {
