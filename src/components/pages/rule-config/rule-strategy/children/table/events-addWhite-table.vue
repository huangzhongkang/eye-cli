    ...
    <el-table
      ...
      <el-table-column
        ...
        <template slot-scope="scope">
          <div :class="[scope.row.rule === '远程溢出攻击[104406]'?'btn btn-warning btn-xs':'btn btn-primary btn-xs']">{{scope.row.rule}}</div>
        </template>
      </el-table-column>
      <el-table-column
        label="加白时间"
      ...
      <el-table-column
        ...
        prop="">
        <template slot-scope="scope">
          <div class="btn btn-danger btn-xs"><i class="fa fa-trash"></i>&nbsp;取消策略</div>
        </template>
      </el-table-column>
    </el-table>
    <div class="page-wrap">
...
<script>
  export default {
    ...
    methods: {
      /** 用户列表初始化数据 **/
      InitUserList() {
        this.$http.get('../../../../../../static/json/events-add-white.json').then(res => {
